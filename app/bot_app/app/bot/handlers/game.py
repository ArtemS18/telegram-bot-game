...
class GameHandler:
    ...
    async def end_game(self, send_message: SendMessage) -> None:
        ms = await self.telegram.send_message(send_message)
        try:
            team = await self.asyncio.start_timer_team(ms.chat.id)
            self.app.log.info(team)
        except asyncio.TimeoutError:
            new_game = await self.db.get_game_by_chat_id(ms.chat.id)
            if new_game:
                send = SendMessage(
                    chat_id=ms.chat.id,
                    text="Лобби удалено из-за превышения лимита ожидания (30 сек)",
                )
                await self.db.update_game(new_game.id, status=GameStatus.end)
            ...
            await self.fsm.set_state(ms.chat.id, self.states.none)
